  ...
  function callback(error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        if(clent === null  ){return res.send(body);}
            services.practice.createPractice(req.body, clent.upn, function(err, data) {
...
function editPractice(req, res, next) {
  ...
  function callback(error, response, body) {
      if (!error && response.statusCode == 200) {
        var clent = JSON.parse(body);
        if(clent === null  ){return res.send(body);}
            services.practice.editPractice(req.params.id, req.body, clent.upn, function(err, data) {
              if (err) { return res.send({error: err});}
              res.send(data);
            });
        return null;
...
function deletePractice(req, res, next) {
  ...
  function callback(error, response, body) {
      if (!error && response.statusCode == 200) {
        var clent = JSON.parse(body);
        if(clent === null  ){return res.send(body);}
            services.practice.deletePractice(req.params.id, clent.upn, function(err, data) {
              if (err) { return res.send({error: err});}
              res.send(data);
            });
        return null;
