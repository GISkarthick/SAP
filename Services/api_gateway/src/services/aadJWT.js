...
aadJwt.validateRequest = function (req, res, callback) {
    try {
        ...
        console.log(config.authority + '/.well-known/openid-configuration');
                ...
                request(result.jwks_uri, function (errorKey, responseKey, bodyKey) {
                    ...
                    resultKeys.keys.forEach(function (key) {
                        key.x5c.forEach(function (keyItem) {
                            ...
                            try {
                                //Attempt to decode
                                // res.send(aadJwt.convertCertificate(secret));
                                // console.log(token, aadJwt.convertCertificate(secret),key.kty);
                                var decoded = jwt.decode(token, aadJwt.convertCertificate(secret),key.kty);
                                req.user=decoded;
                                ...
                                if (result.issuer === decoded.iss &&
                                    ...
                                    ((new Date().getTime() / 1000 ) <parseFloat(decoded.exp) )) {
                                    //All valid, we can proceed
                                    validated = true;
                                }
                            }
                            catch (ex) {console.log('ex');console.log(ex);
                            }
                        });
                    });
                    callback(validated);
        ...
        callback(false);console.log('e');console.log(e);
    }
